...
class UserLogProvider with ChangeNotifier {
  ...
  @override
  void getPostLogUser(String myUserId) async {
    alllog = await userLog(myUserId);
    alluser = await allUser(myUserId);
    await addLogUser();
    notifyListeners();
  }

  Future addLogUser() async {
    try {
      this.actionTxt = [];
      this.dateformat1 = [];
      this.first_name = [];
